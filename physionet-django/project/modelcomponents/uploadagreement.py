...
class UploadAgreement(models.Model):
    ...
    human_subjects_deidentified = models.BooleanField(
        default=False,
        help_text='This project contains data obtained from human subjects, and all personally identifiable information has been removed'
    )
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                check=models.Q(no_human_subjects=True) | 
    ...
    def __str__(self):
        return f"Upload Agreement for {self.project.title} - {'Accepted' if self.accepted else 'Pending'}"
